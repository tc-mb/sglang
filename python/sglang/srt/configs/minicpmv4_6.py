...
"""MiniCPM-V 4.6 configuration (nested text_config + vision_config)."""
...
class MiniCPMV4_6Config(PretrainedConfig):
    """Top-level config for ``MiniCPMV4_6ForConditionalGeneration``.

    Mirrors the vLLM config class in
    ``vllm.transformers_utils.configs.minicpmv4_6`` so the same HF checkpoint
    can be loaded in either engine.
    """
